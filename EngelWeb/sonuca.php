...
$sonucsorgu =mysqli_query($conn,"SELECT * FROM ilangonderi WHERE mail LIKE '%".$aramasorgusu."%'" );
if(mysqli_num_rows($sonucsorgu)>0){
 while($sorguoku=mysqli_fetch_array($sonucsorgu)){
  ...
  echo "Arama Sonucu Açığa Çıkan ilangonderi"," ", $sorguoku['ilangonderi'].'<br>';
 }
}
else{
...
<input type="button" onclick="window.location.href = 'ilangoruntule.php'" value="Geri Gel" class="button">
</form>
</center>
<style>
  .button {
